...
app.get('/api/f1/results', async (req, res) => {
  ...
  try {
    // Step 1 get all 2024 race sessions
    ...
    // positions driver details in parallel
    ...
    if (!posRes.ok) throw new Error(`OpenF1 position: HTTP ${posRes.status}`);
    if (!drvRes.ok) throw new Error(`OpenF1 drivers: HTTP ${drvRes.status}`);

    const positions  = await posRes.json();
    const driverList = await drvRes.json();
